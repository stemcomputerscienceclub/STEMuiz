...
export const db = {
  ...
  async getGameSession(sessionId) {
    try {
      console.log(`Fetching game session with ID: ${sessionId}`);
      ...
      if (checkError) {
        console.error('Error checking game session:', checkError);
        throw new Error(`Game session not found: ${sessionId}`);
      }
      
      // Then try to get the full session data with relationships
      try {
        const { data: session, error: sessionError } = await supabase
          .from('game_sessions')
          .select(`
        ...
        if (sessionError) {
          console.error('Error fetching game session with relationships:', sessionError);
          throw sessionError;
        }
        
        // Get players separately if the nested query is causing issues
        ...
        console.log(`Successfully loaded game session: ${sessionId}`);
        return session;
      } catch (nestedError) {
        console.error('Error with nested query, trying simplified approach:', nestedError);
        
        // Try a simpler approach without nested queries
        const { data: session, error: sessionError } = await supabase
          .from('game_sessions')
          .select('*')
          .eq('id', sessionId)
        ...
        console.log(`Successfully loaded game session using simplified approach: ${sessionId}`);
        return session;
      }
    } catch (error) {
